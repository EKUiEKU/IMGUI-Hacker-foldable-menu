...
void ImGui::_SecondItem(const char* title, const char* status, const int color, const int bg_color, ImVec2 size, bool open, bool selected) {
    va_list args;
    va_start(args, title);
    ...
    ImGuiContext& g = *GImGui;
    const char* title_text_end = g.TempBuffer + ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), title, args);
    const char* status_text_end = g.TempBuffer + ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), status, args);
    const char* text_begin = g.TempBuffer;

    const float wrap_pos_x = window->DC.TextWrapPos;
    const bool wrap_enabled = (wrap_pos_x >= 0.0f);
    const float wrap_width = wrap_enabled ? CalcWrapWidthForPos(window->DC.CursorPos, wrap_pos_x) : 0.0f;
    const ImVec2 title_text_size = CalcTextSize(text_begin, title_text_end, false, wrap_width);
    const ImVec2 status_text_size = CalcTextSize(text_begin, status_text_end, false, wrap_width);
    ...
    ImRect bb(text_pos, text_pos + title_text_size);
    ItemSize(title_text_size, 0.0f);
    if (!ItemAdd(bb, 0))
        return;
    ...
    auto MaxRect = text_pos + size;
    //背景颜色
    window->DrawList->AddRectFilled(text_pos, { MaxRect.x , MaxRect.y }, bg_color);

    if(selected)
        window->DrawList->AddRect(text_pos, { MaxRect.x , MaxRect.y }, 0xff0000ff);


    auto statusMin = MaxRect - status_text_size;
    float d = (height - blockSize.y) / 2.f;
    auto titleContainerWidth = statusMin.x - (bb.Min.x + 2 * d + blockSize.x);
    auto titleStartX = bb.Min.x + titleContainerWidth / 2.f - title_text_size.x / 2.f;
    ...
    //开启状态 方块
    window->DrawList->AddRectFilled({ text_pos.x + d, bb.Min.y + d }, { text_pos.x + blockSize.x + d, text_pos.y + d + blockSize.y}, blockColor);
    //功能的名称
    window->DrawList->AddText( { titleStartX, bb.Min.y}, color, title);
    //功能的状态
    window->DrawList->AddText({ statusMin.x, bb.Min.y }, color, status);

    va_end(args);
}

IMGUI_API void ImGui::_FirstItem(const char* title, const char* status,const int color, const int bg_color,ImVec2 size, bool selected)
{
    va_list args;
    va_start(args, title);
    ...
    auto MaxRect = bb.Min + size;

    auto statusMin = MaxRect - status_text_size;

    auto titleContainerWidth = statusMin.x - bb.Min.x;
    //背景颜色
    ...
    auto title_startX = bb.Min.x + titleContainerWidth / 2.0f - title_text_size.x / 2.0f;

    //功能的名称
    window->DrawList->AddText({ title_startX, bb.Min.y }, color,title);
    //功能的状态
    window->DrawList->AddText({ statusMin.x, bb.Min.y }, color, status);
...
IMGUI_API void ImGui::_Title(const char* title, const int color, const int bg_color, ImVec2 size)
{
    va_list args;
    va_start(args, title);
    ...
    auto titleStartX = bb.Min.x + size.x / 2.0f - text_size.x / 2.0f;
    //背景颜色
    window->DrawList->AddRectFilled(bb.Min, MaxRect, bg_color);

    //功能的名称
    window->DrawList->AddText({ titleStartX, bb.Min.y }, color, text_begin, text_end);
...
IMGUI_API void ImGui::SecondItem(const char* title, const char* status, bool open, bool selected)
{
    ImGui::_SecondItem(title, status, 0xffFFFfff, 0x77000000, { 150,16 }, open, selected);
}

IMGUI_API void ImGui::FirstItem(const char* title, const char* status, bool selected)
{
    ImGui::_FirstItem(title, status , 0xffFFF500, 0x77000000, { 150,16 }, selected);
}


IMGUI_API void ImGui::Title(const char* title)
{
    ImGui::_Title(title, 0xffFFFfff, 0x77000000, { 150,16 });
}